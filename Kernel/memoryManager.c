...
void *mallocMM(unsigned nbytes) {
  ...
  for (currNode = prevNode->data.ptr;;
       prevNode = currNode, currNode = currNode->data.ptr) {
...
void freeMM(void *freeMem) {
  ...
  for (currNode = startingNode;
       !(freeBlock > currNode && freeBlock < currNode->data.ptr);
       currNode = currNode->data.ptr) {

    if (freeBlock == currNode || freeBlock == currNode->data.ptr)
      return;

    if (currNode >= currNode->data.ptr &&
        (freeBlock > currNode || freeBlock < currNode->data.ptr)) {
      isExternal = 1;
      break;
    }
  ...
  if (!isExternal &&
      (currNode + currNode->data.size > freeBlock ||
