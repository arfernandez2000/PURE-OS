...
void initScheduler(uint64_t rsp) { entre1era = rsp; }
...
uint64_t scheduler(uint64_t lastRSP) {
  ...
  do {
    ...
    if (stopInanition == 0) {
      ...
      if (processQueue[currentProcess]->ppid != 0 &&
          processQueue[processQueue[currentProcess]->ppid]->state != READY) {
        phylo =
            1; // awful fix but we are running out of time . Works temporarily
        continue;
      }
...
int addProcess(void (*entryPoint)(int, char **), int argc, char **argv, int fg,
  ...
  processQueue[activeProcesses] =
      createPCB(entryPoint, argc, argv, fg, fd, name);
  return processID++;
}
...
PCB *createPCB(void (*entryPoint)(int, char **), int argc, char **argv, int fg,
  ...
  if (strcmp(name, "phylo") == 0) {
    newProcess->ppid =
        atoi(newProcess->argv[1], Stringlen(newProcess->argv[1]));
  } else {
    newProcess->ppid = 0;
  }
...
void newProcessStack(void(*fn), int argc, char **argv, PCB *newProcess) {
  ...
  newProcess->rsp =
      (uint64_t)_initialize_stack_frame(fn, newStack + STACK_SIZE, argc, argv);
...
uint64_t preserveStack(uint64_t rsp) {
  ...
  if (++currentProcess >= activeProcesses)
    currentProcess = 0;
...
int getProcessCount() { return activeProcesses; }

uint64_t getPID() { return processQueue[currentProcess]->pid; }

uint64_t getPPID() { return processQueue[currentProcess]->ppid; }

int *getPipes() { return processQueue[currentProcess]->pipes; }
...
char **psDisplay() {
  ...
  for (int i = 0; i < activeProcesses; i++) {
    ...
    strcat(processString[i],
           itoa((uint64_t)processQueue[i]->rbp, buff, 10, 10));
    strcat(processString[i], "   ");
    strcat(processString[i], "0x");
...
void saveSampleRSP(uint64_t rsp) { sampleRSP = rsp; }

uint64_t getSampleRSP() { return sampleRSP; }
...
int unBlockProcess(uint64_t pid) { return changeState(pid, READY); }
